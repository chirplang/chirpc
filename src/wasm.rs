...
pub struct LocalMap {
    pub names: HashMap<String, (LocalId, ChipType)>
}
...
#[derive(Clone)]
pub enum ChipType {
    Struct(Rc<HashMap<String, ChipType>>),
    Primitive(Primitive)
}

impl ChipType {

    pub fn flatten(&self, name: &str) -> HashMap<String, Primitive> {
    ...
    fn flatten_inner(name: &str, type_: &ChipType, map: &mut HashMap<String, Primitive>) {
        match type_ {
...
pub fn compile_statement_wasm<'a>(builder: &mut InstrSeqBuilder, func_locals: &'a mut LocalMap, module_locals: &mut ModuleLocals, statement: &Statement) -> Option<ChipType> {
    match statement {
        ...
        Statement::LetAssign(ident, statement) => {
            ...
            primitives.iter().collect::<Vec<(&String, &Primitive)>>().iter().rev().for_each(|&(k, v)| {
                let local = module_locals.add(match v {
                    Primitive::F64 => ValType::F64,
                    Primitive::I64 => ValType::I64
                });

                func_locals.names.insert(k.into(), (local, ChipType::Primitive(*v)));
